...
cmd="./ycsbc -db pmhashtable -threads 1 -P $load -load true -dbstatistics true -dbwaitforbalance false >>out.out 2>&1 "
echo $cmd >out.out
echo $cmd
eval $cmd
if [ $? -ne 0 ];then
...
for file_name in $workloads; do
  CLEAN_CACHE
  cmd="./ycsbc -db pmhashtable -threads 1 -P $file_name -run true -dbstatistics true -dbwaitforbalance false >>out.out 2>&1 "
  echo $cmd >>out.out
  echo $cmd
  eval $cmd
  if [ $? -ne 0 ];then
